...
var customScriptPopupJs = {
		
	init : function(){
		var projectId = $('[name="projectId"]').val();
		//console.log('projectId',projectId);
		customScriptPopupJs.getCustomScriptContents();			// get customScript info
	},
	getCustomScriptContents : function () {
		var projectId = $('[name="projectId"]').val();
		var customScriptId = $('[name="customScriptId"]').val();
		
		console.log("projectId",projectId);
		console.log("customScriptId",customScriptId);
		var urlVal = '/popup/getCustomScriptContents';
		$.ajax({
			url   		: urlVal,
			type  		: "post",
			dataType    : "json",
			...
			success     : function(responseData){
				console.log("getCustomScriptContents data",responseData);
				if (null != responseData) {
					customScriptPopupJs.setCustomScriptListHtml(responseData.slCustomScript);
				}
			},
			error : function(e){}
		});
	},
	setCustomScriptListHtml : function (slCustomScript) {
		var customScriptContents = slCustomScript.customScriptContents;
		var lines = slCustomScript.customScriptContents.split("\n");
		var space = "";
		var spaceHtml = "";
		var contentsHtml = "";
...
//			console.log('lines['+i+'].replace(/</g, "&lt;")', lines[i].replace(/</g, "&lt;"));
			contentsHtml += spaceHtml + lines[i].replace(/</g, "&lt;") + "<br>";
			spaceHtml = "";
		}
...
//		console.log('contentsHtml',contentsHtml);		
		$('#setBoostBody').html(contentsHtml);
	},/*
